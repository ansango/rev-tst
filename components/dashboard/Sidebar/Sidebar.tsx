...
const LinkBlock: FC<{ path: string; icon: any; label: string }> = ({
  path,
  icon,
  label,
}) => {
        ...
        <ul className="space-y-2">
          {routes.map(({ label, path, icon }) => {
            if (path === "/dashboard/settings") {
              const url = `/dashboard/settings/${user?.username}`;
              return <LinkBlock {...{ label, path: url, icon }} key={path} />;
            }
            return <LinkBlock {...{ label, path, icon }} key={path} />;
          })}
        </ul>
      </div>
    </aside>
